...
def get_reporting_manager(first_name, last_name=''):
    cur = conn.cursor()
    try:
        stm = "select first_name, last_name, reporting_lead from public.enq_emp_details where lower(first_name) = '%s' or lower(last_name) = '%s' " % (
...
def get_team_members(myfirstname, mylastname):
    cur = conn.cursor()
    try:
        query = "select account, business_unit_description, reporting_lead, delivery_lead from public.enq_emp_details where lower(first_name) = '%s' and lower(last_name) = '%s'" % (
        myfirstname, mylastname)
        df = pd.read_sql_query(query, conn)
        print(df['account'][0])
        print('--------------')
        team_member_query = "select concat(first_name, ' ' , last_name) from public.enq_emp_details where lower(account) = '%s'  and lower(business_unit_description) = '%s' and lower(reporting_lead) = '%s' and lower(delivery_lead) = '%s' and lower(first_name) != '%s' and lower(last_name) != '%s' " % (
        df['account'][0].lower(), df['business_unit_description'][0].lower(), df['reporting_lead'][0].lower(),
        df['delivery_lead'][0].lower(), myfirstname.lower(), mylastname.lower())
...
def isvaliduser():
    ...
    cur = conn.cursor()
    try:
        stm = "select first_name, last_name from public.enq_emp_details where lower(first_name) = '%s' and lower(last_name) = '%s' " % (
        first_name.lower(), last_name.lower())
        cur.execute(stm)
        emp = cur.fetchall()
        if len(emp) == 0:
